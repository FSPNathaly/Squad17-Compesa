...
export interface ImportedFileEntry {
  ...
  perdasMunicipioData: { name: string, manutencao: number, faltaAgua: number, semAcesso: number }[];
...
const App: React.FC = () => {
  ...
  const [importedFilesHistory, setImportedFilesHistory] = useState<ImportedFileEntry[]>([]);
  const [activeFileId, setActiveFileId] = useState<string | null>(null);

  const activeFileEntry = activeFileId ? importedFilesHistory.find(f => f.id === activeFileId) : null;
  ...
  const navigateTo = useCallback((page: Page) => {
    if (page === 'home') {
      ...
      if (page === 'planilha' && !activeFileId && importedFilesHistory.length > 0) {
      }
      setCurrentPage(page);
    } else {
      setCurrentPage('home');
  ...
  const handleFileImportedOrUpdated = useCallback((fileEntry: ImportedFileEntry) => {
  ...
  const handleFileDeleted = useCallback((fileIdToDelete: string) => {
    setImportedFilesHistory(prev => prev.filter(f => f.id !== fileIdToDelete));
    if (activeFileId === fileIdToDelete) {
      setActiveFileId(null);
    }
  }, [activeFileId]);
  
  const handleFileDateEdited = useCallback((fileIdToEdit: string, newDate: Date) => {
    setImportedFilesHistory(prev => 
      prev.map(f => f.id === fileIdToEdit ? { ...f, importDate: newDate } : f)
    );
  }, []);
  ...
  useEffect(() => {
    ...
    if (storedFiles) {
      try {
        const parsedFiles: ImportedFileEntry[] = JSON.parse(storedFiles).map((file: any) => ({
          ...file,
  ...
  useEffect(() => {
    if (importedFilesHistory.length > 0) {
      localStorage.setItem('compesaDashboardFiles', JSON.stringify(importedFilesHistory));
    } else {
      localStorage.removeItem('compesaDashboardFiles');
    }
