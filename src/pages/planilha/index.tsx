...
export const Planilha = ({
  ...
  const processAndStoreFileDataInPlanilha = useCallback(
    (
      fileId: string,
      fileName: string,
      importDate: Date,
  ...
  const handleFileUploadInPlanilha = useCallback(
    (
      ...
      Papa.parse(file, {
        ...
        complete: (results: ParseResult<CsvData>) => {
          ...
          processAndStoreFileDataInPlanilha(
            fileIdToUse,
            file.name,
            dateToUse,
            parsedData
  ...
  const handleMainImportClickPlanilha = () =>
    mainFileInputRefPlanilha.current?.click();
    ...
    if (sortConfig !== null) {
      sortableData.sort((a, b) => {
        ...
        const numA = parseFloat(
          String(valA).replace(/\./g, "").replace(",", ".")
        );
        const numB = parseFloat(
          String(valB).replace(/\./g, "").replace(",", ".")
        );
  ...
  const tableHeaders = useMemo(
    () => getTableHeaders(activeFileRawData || []),
    [activeFileRawData]
  );
  ...
  const handleExportCSV = () => {
    ...
    link.download = `${
      activeFileName ? activeFileName.split(".")[0] : "planilha"
    }_export_${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
    ...
    return (
      <div className="min-h-screen bg-[#F0F5FF] p-6 flex flex-col items-center justify-center text-center">
              ...
              >
                <Avatar className="w-10 h-10 border-2 border-[#85A6F2]">
                  <AvatarImage
                    ...
                    alt="User Avatar"
                  />
                  <AvatarFallback className="bg-[#85A6F2] text-white font-medium">
                    AD
                  </AvatarFallback>
                </Avatar>
              </Button>
            ...
            <DropdownMenuContent align="end" className="w-56 bg-white">
              <DropdownMenuLabel>
                <div className="flex flex-col">
                  <span className="font-medium text-[#1A2C56]">
                    Administrador
                  </span>
        ...
        <p className="text-gray-500 mb-4">
          {importedFilesHistory.length === 0
            ? "Importe um arquivo CSV no Dashboard ou aqui para começar."
            : "Selecione um arquivo na lista abaixo para visualizar na planilha."}
        </p>
        {(importedFilesHistory.length > 0 || uploading) && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                <FileText className="h-3 w-3 sm:h-4 sm:w-4" /> Selecionar
                Arquivo ({importedFilesHistory.length}){" "}
        ...
        <Button
          variant="outline"
          className="mt-4 gap-1 sm:gap-2 border-[#003F9C] text-[#003F9C] hover:bg-[#003F9C]/10 text-xs sm:text-sm px-2 sm:px-3"
          onClick={handleMainImportClickPlanilha}
        >
          <Upload className="h-3 w-3 sm:h-4 sm:w-4" /> Importar Novo Arquivo
        </Button>
        <input
          type="file"
          ref={mainFileInputRefPlanilha}
          accept=".csv"
          onChange={(e) => handleFileUploadInPlanilha(e)}
          className="hidden"
        />
        <input
          type="file"
          ref={editFileInputRefPlanilha}
          accept=".csv"
          onChange={(e) => {
            if (fileToEditId) handleFileUploadInPlanilha(e, fileToEditId);
          }}
          className="hidden"
        />
        {uploading && (
    ...
    <div className="min-h-screen bg-[#F0F5FF] p-6">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 bg-white p-4 rounded-lg shadow-sm">
        <div className="flex items-center gap-2 md:gap-4 flex-wrap">
          ...
          <div className="space-y-1 flex-grow min-w-0">
            <h2
              className="text-lg sm:text-xl font-semibold text-[#1A2C56] truncate"
              title={activeFileName || "Arquivo Desconhecido"}
            >
            ...
            <p className="text-xs sm:text-sm text-gray-500">
              Dados de:{" "}
              {activeFileDate
                ? format(activeFileDate, "dd/MM/yyyy HH:mm")
                : "Data Desconhecida"}
            </p>
          </div>
        </div>
                      ...
                      <DropdownMenuContent side="right" align="start">
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            ...
                            >
                              <CalendarIcon className="mr-2 h-4 w-4" /> Editar
                              Data
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent
                        ...
                        <DropdownMenuItem
                          onClick={() => handleTriggerEditFilePlanilha(file.id)}
                        >
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <Button
          ...
          className="gap-2 bg-[#003F9C] text-white hover:bg-[#00337A]"
        >
            ...
            {paginatedData.map((row, rowIndex) => (
              ...
              >
                {tableHeaders.map((header, cellIndex) => (
                  ...
                  >
                    {row[header as keyof CsvData] !== undefined &&
                    row[header as keyof CsvData] !== null
                      ? String(row[header as keyof CsvData])
                      : "-"}
                  </TableCell>
                ))}
              </TableRow>
